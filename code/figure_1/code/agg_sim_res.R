...
for(row in 1:nrow(sim_mat)) {
  sim_row <- sim_mat[row,]
  for(method in c("ancombc2", "linda", "maaslin2", "NBZIMM", "lmerseq", "aldex", "aldex_lin")) {
    for(ind in 1:20) {
      e <- tryCatch({
...
colnames(all_res) <- c("method", "n_s", "i", "system", "pc_l", "pc_u", "TP", "NTP", "FP", "FN", "TN", "FDR", "Power")
...
all_res <- all_res %>% group_by(n_s, method, system, pc_l, pc_u) %>% summarize(FDR_mean=mean(FDR),
